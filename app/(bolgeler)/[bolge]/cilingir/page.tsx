...
export function generateMetadata({ params }: { params: { bolge: string } }) {
  ...
  return {
    title: `${region.name} Çilingir | 7/24 Acil Hizmet — Çelik Çilingir`,
    description: `${region.name}'de 7/24 kapı açma, kilit değişimi ve acil çilingir hizmeti. 15 dakikada kapınızdayız. Hemen arayın.`,
    alternates: {
      canonical: `https://celikcilingir.com/${region.slug}/cilingir`,
    },
    openGraph: {
      title: `${region.name} Çilingir | 7/24 Acil Hizmet`,
      description: `${region.name}'de 7/24 kapı açma, kilit değişimi ve acil çilingir hizmeti. 15 dakikada kapınızdayız.`,
      url: `https://celikcilingir.com/${region.slug}/cilingir`,
    }
  };
}
...
export default function BolgeCilingirPage({ params }: { params: { bolge: string } }) {
  const region = regions.find((r) => r.slug === params.bolge);
  ...
  const faqs = [
    {
      q: `${region.name}'de çilingir fiyatları ne kadar?`,
      a: "Hizmet türüne göre değişmekle birlikte (çekili kapı, kilitli kapı, barel değişimi vb.) uygun ve rekabetçi fiyatlar sunmaktayız. Net fiyat için telefonda bilgi alabilirsiniz."
    },
    {
      q: `${region.name}'ye kaç dakikada geliyorsunuz?`,
      a: "Ortalama 15-20 dakika içinde adresinizde oluyoruz. Gezici mobil araçlarımız sayesinde trafiğe takılmadan hızlıca ulaşıyoruz."
    },
    {
    ...
    {
      q: `${region.name}'nin hangi mahallelerine hizmet veriyorsunuz?`,
      a: `Şu mahallelere hizmet vermekteyiz: ${region.neighborhoods.join(", ")}.`
    }
  ];
  ...
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "name": `Çelik Çilingir - ${region.name}`,
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {region.neighborhoods.map((mahalle) => {
              ...
              return (
                <Link
                  key={mahalle}
                  href={`/${region.slug}/${mahalleSlug}/cilingir`}
