...
export function generateMetadata({ params }: { params: { bolge: string; mahalle: string } }) {
  ...
  return {
    title: `${mahalle} Oto Anahtarcı | ${region.name} — Çelik Çilingir`,
    description: `${region.name} ${mahalle} mahallesinde 7/24 oto anahtarcı hizmeti. Araç kapısı açma, kontak kopyalama, çip anahtar. Hemen arayın.`,
    alternates: {
      canonical: `https://celikcilingir.com/${region.slug}/${params.mahalle}/oto-anahtarci`,
    },
    openGraph: {
      title: `${mahalle} Oto Anahtarcı | ${region.name} Çelik Çilingir`,
      description: `${region.name} ${mahalle} mahallesinde 7/24 oto anahtarcı hizmeti. Araç kapısı açma, kontak kopyalama, çip anahtar.`,
      url: `https://celikcilingir.com/${region.slug}/${params.mahalle}/oto-anahtarci`,
    }
  };
}
...
export default function MahalleOtoAnahtarciPage({ params }: { params: { bolge: string; mahalle: string } }) {
  const region = regions.find((r) => r.slug === params.bolge);
  if (!region) {
    notFound();
  }
  ...
  const faqs = [
    {
      q: `${mahalle}'de oto anahtarcı fiyatları ne kadar?`,
  ...
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "name": `Çelik Çilingir Oto Anahtarcı - ${mahalle} Mahallesi`,
      ...
      <section className="relative pt-24 pb-8 md:pt-28 md:pb-16 bg-background overflow-hidden flex flex-col items-center justify-center text-center">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 w-full">
          <nav className="flex items-center justify-center gap-2 text-sm font-medium text-slate-400 mb-4 overflow-x-auto whitespace-nowrap">
            ...
            <span>/</span>
            <Link href={`/${region.slug}/oto-anahtarci`} className="hover:text-primary transition-colors">{region.name}</Link>
            <span>/</span>
      ...
      <section className="py-20 bg-surface">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-slate-400 text-lg leading-relaxed mb-8">
            {region.name} {mahalle} bölgesinde aracınızın kapısı mı kilitli kaldı? Uzman oto çilingir ekibimiz, en kısa sürede adresinize ulaşarak aracınıza hasar vermeden işlemleri tamamlar.
          </p>
